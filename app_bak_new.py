...
from supabase_modules.auth import register_user, login_user, logout_user, get_current_user, verify_session, require_auth, change_password, reset_password_request
...
def answer_question(question, top_k=20, threshold=5.0, model="gemini-2.0-flash"):
    """
    Trả lời câu hỏi dựa trên các tài liệu đã tải lên
    
    Args:
        question (str): Câu hỏi của người dùng
        top_k (int): Số lượng đoạn văn bản phù hợp nhất để lấy
        threshold (float): Ngưỡng điểm số tương đồng để lọc các kết quả
        model (str): Model Gemini để sử dụng
        
    Returns:
        dict: Kết quả bao gồm câu trả lời và nguồn tài liệu
    """
    # Kiểm tra xem global_metadata có dữ liệu không
    if not global_metadata or not global_all_files:
        return {
            "answer": "Không có tài liệu nào được tải lên. Vui lòng tải lên tài liệu trước khi đặt câu hỏi.",
            "sources": []
        }
    
    # Đảm bảo đã load dữ liệu của người dùng hiện tại
    ...
    if len(questions) > 1:
        ...
        for i, single_question in enumerate(questions):
            ...
            result = answer_question(single_question, top_k, threshold, model)
...
"""
    ...
    # Theo dõi hiệu suất
    track_performance(question, retrieval_time, answer_time, len(retrieved_chunks), None)
...
def load_state():
    ...
    try:
        ...
        logger.info(f"Đã nạp thành công trạng thái hệ thống từ {state_file}")
        return True
    except Exception as e:
        logger.error(f"Lỗi khi nạp trạng thái: {str(e)}")
        return False
...
def query():
    ...
    if not global_all_files:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return render_template_string(
                """<div class="chat-message bot-message"><p class="text-gray-800 dark:text-gray-200 text-sm sm:text-base">Vui lòng tải lên ít nhất một tài liệu trước khi đặt câu hỏi.</p></div>"""
            )
        else:
            return render_template_string(
                index_html,
                files=global_all_files,
                settings=load_settings(),
                answer="Vui lòng tải lên ít nhất một tài liệu trước khi đặt câu hỏi.",
                sources=None
            )
