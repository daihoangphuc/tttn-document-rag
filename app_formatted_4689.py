...
from supabase_modules.auth import (
    register_user,
    login_user,
    logout_user,
    get_current_user,
...
from supabase_modules.chat_history import (
    create_chat,
    get_chats,
    get_chat,
    update_chat_title,
...
from supabase_modules.file_manager import (
    ensure_user_upload_dir,
    save_user_file,
    get_user_files,
    delete_user_file,
    migrate_files_to_user_directory,
    get_file_path,
)
from supabase_modules.helpers import (
    get_user_id_from_session,
    migrate_localStorage_to_supabase,
    initialize_user_data,
    format_chat_history_for_frontend,
    get_user_files_with_metadata,
)
...
from supabase_integration import (
    ...
    enhanced_remove_file,
    get_enhanced_index_html,
)
    ...
    while retries < max_retries:
        try:
            ...
            response = gemini_model.generate_content(
                prompt, generation_config=config_copy
            )
            ...
            error_message = str(e)
            logger.error(
                f"Lỗi khi gọi Gemini API (lần thử {retries+1}/{max_retries}): {error_message}"
            )
...
def fix_vietnamese_spacing(text):
    ...
    7. Giữ nguyên các dấu xuống dòng và khoảng cách giữa các đoạn văn"""
        ...
        fixed_text = generate_with_retry(
            prompt,
            {
                "temperature": 0.1,
                "top_p": 0.95,
...
def extract_text_txt(file_path):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Lỗi khi đọc file TXT {file_path} với encoding latin-1: {str(e)}"
            )
            return ""
    except Exception as e:
    ...
    try:
        ...
        for i, para in enumerate(paragraphs):
            ...
            is_section_header = any(
                re.match(pattern, para.strip(), re.MULTILINE)
        ...
        for chunk in final_chunks:
            ...
            if (
                chunk_word_count > max_words * 1.5
            ):  # Nếu chunk lớn hơn 150% kích thước tối đa
    ...
    try:
        ...
        def parse_hierarchical_structure(text, level=0, path=None):
            ...
            for i, match in enumerate(matches):
                ...
                sub_chunks = parse_hierarchical_structure(
                    content, level + 1, current_path
                )
        ...
        try:
            ...
            paragraphs = re.split(r"\n\s*\n", text)
            hierarchical_chunks = [
                {"text": para, "path": []} for para in paragraphs if para.strip()
            ]
...
def create_contextual_chunks(
    text, max_chunk_size=1000, min_chunk_size=100, overlap_ratio=0.2
):
    """
...
def create_multi_granularity_chunks(text, config=None):
    ...
    try:
        ...
        if not sections and len(text) > config["max_section_length"]:
            words = text.split()
            section_size = min(
                len(words), config["max_section_length"] // 5
            )  # Ước tính 5 ký tự/từ
...
def create_hybrid_chunks(text, config=None):
    ...
    try:
        ...
        multi_granularity_chunks = create_multi_granularity_chunks(
            text, config.get("multi_granularity")
        )
        ...
        sorted_chunks = sorted(
            unique_chunks, key=lambda x: abs(len(x) - 1000)
        )  # 1000 là độ dài lý tưởng
...
def load_state():
    ...
    try:
        ...
        if os.path.exists(vectors_file):
            ...
            if os.path.exists(tfidf_vectorizer_file) and os.path.exists(
                tfidf_matrix_file
            ):
        ...
        logger.info(
            f"Đã nạp dữ liệu từ {load_dir}: {len(global_metadata)} chunks, {len(global_all_files)} file"
        )
        return True
    except Exception as e:
...
def rebuild_indices():
    ...
    try:
        ...
        if len(all_texts) < 2:
            logger.warning(
                "Số lượng tài liệu quá ít để xây dựng TF-IDF index. Cần ít nhất 2 tài liệu."
            )
        ...
        try:
            ...
            logger.info(
                f"Đã xây dựng lại TF-IDF index với {tfidf_matrix.shape[0]} documents và {tfidf_matrix.shape[1]} features"
            )
        except Exception as e:
            ...
            simple_vectorizer = TfidfVectorizer(
                lowercase=True, min_df=1, max_df=1.0, ngram_range=(1, 1)
            )
            new_matrix = simple_vectorizer.fit_transform(all_texts)
            tfidf_vectorizer = simple_vectorizer
            tfidf_matrix = new_matrix
            logger.info(
                f"Đã xây dựng TF-IDF index đơn giản với {tfidf_matrix.shape[0]} documents"
            )
    ...
    if tfidf_vectorizer is None:
        ...
        try:
            ...
            tfidf_matrix = tfidf_vectorizer.fit_transform(
                [meta["text"] for meta in global_metadata]
            )
...
def evaluate_performance(test_queries, top_k=5, chunking_method=None):
    ...
    for query in test_queries:
        ...
        answer, sources = answer_question(
            query, top_k=top_k, chunking_method=chunking_method
        )
        ...
        if query_perf:
            result = {
                ...
                "chunking_method": query_perf.get(
                    "chunking_method", chunking_method or "unknown"
                ),
            }
    ...
    avg_retrieval_time = (
        sum(r["retrieval_time"] for r in results) / len(results) if results else 0
    )
    avg_answer_time = (
        sum(r["answer_time"] for r in results) / len(results) if results else 0
    )
    avg_total_time = (
        sum(r["total_time"] for r in results) / len(results) if results else 0
    )
...
def analyze_performance(chunking_method=None):
    ...
    if not global_performance:
        return {"message": "Chưa có dữ liệu hiệu suất", "data": {}}

    # Lọc theo phương pháp chunking nếu được chỉ định
    filtered_data = global_performance
    if chunking_method:
        filtered_data = [
            p for p in global_performance if p.get("chunking_method") == chunking_method
        ]
    ...
    # Tính toán các chỉ số trung bình
    avg_retrieval_time = sum(p["retrieval_time"] for p in filtered_data) / len(
        filtered_data
    )
    ...
    for method, data in chunking_methods.items():
        if data["count"] > 0:
            ...
            data["avg_total_time"] = (
                data["total_retrieval_time"] + data["total_answer_time"]
            ) / data["count"]
    ...
    analysis = {
        ...
        "recent_queries": [
            ...
            for p in sorted(
                filtered_data, key=lambda x: x.get("timestamp", 0), reverse=True
            )[:5]
        ],
    }

    return {"message": "Phân tích hiệu suất thành công", "data": analysis}


# --- Hàm theo dõi hiệu suất ---
def track_performance_metrics(
    query, retrieval_time, answer_time, chunk_count, chunking_method
):
    ...
    try:
        ...
        logger.info(
            f"Đã lưu metrics hiệu suất, tổng số queries: {len(performance_metrics['queries'])}"
        )
    except Exception as e:
...
def build_optimized_context(chunks, metadata, question, max_tokens=4000):
    ...
    for pattern in long_answer_patterns:
        if re.search(pattern, question.lower()):
            ...
            max_tokens = max(
                max_tokens, 8000
            )  # Tăng lên 8000 tokens cho câu trả lời dài
            break
    ...
    for pattern in exact_answer_patterns:
        if re.search(pattern, question.lower()):
            ...
            max_tokens = max(
                max_tokens, 10000
            )  # Tăng lên 10000 tokens cho câu trả lời chính xác và đầy đủ
            break
    ...
    if requires_overview:
        ...
        for chunk, meta in sorted_chunks_with_metadata:
            ...
            if (
                meta.get("is_law_article", False)
                and meta.get("article_number") in article_numbers
            ):
    ...
    question_keywords = set(
        word.lower() for word in re.findall(r"\b\w+\b", question) if len(word) > 3
    )
    ...
    for i, (chunk, meta) in enumerate(sorted_chunks_with_metadata):
        ...
        keyword_matches = sum(
            1 for keyword in question_keywords if keyword in chunk_text
        )

        # Tính điểm dựa trên số lượng từ khóa khớp và vị trí trong danh sách
        relevance_score = (
            keyword_matches * 10 - i * 0.1
        )  # Ưu tiên các chunks có nhiều từ khóa và xuất hiện sớm
        ...
        if re.search(
            r"(là|được định nghĩa là|được hiểu là|có nghĩa là|được gọi là)", chunk
        ):
            relevance_score += 6
        ...
        if re.search(r"(các|những)[^:]+:", chunk) or re.search(
            r"(\d+\.|[a-z]\)|\([a-z]\))", chunk
        ):
            relevance_score += 7
        ...
        if "[Granularity: document]" in chunk and requires_overview:
            relevance_score += (
                15  # Ưu tiên cao nhất cho document-level chunks khi cần tổng quan
            )
        elif "[Granularity: section]" in chunk and requires_overview:
            relevance_score += (
                10  # Ưu tiên cao cho section-level chunks khi cần tổng quan
            )
        elif "[Granularity: section]" in chunk and requires_long_answer:
            relevance_score += (
                12  # Ưu tiên cao cho section-level chunks khi cần trả lời dài
            )
        elif "[Granularity: paragraph]" in chunk and requires_exact_answer:
            relevance_score += (
                8  # Ưu tiên cho paragraph-level chunks khi cần trả lời chính xác
            )
        elif "[Granularity: sentence]" in chunk and not requires_overview:
            relevance_score += (
                5  # Ưu tiên nhẹ cho sentence-level chunks cho câu hỏi thông thường
            )

        # Tăng điểm cho các chunks có nội dung trải dài qua nhiều trang
        if re.search(r"\[Trang \d+\].*\[Trang \d+\]", chunk):
            relevance_score += (
                10  # Ưu tiên cao cho chunks có nội dung trải dài qua nhiều trang
            )
    ...
    for rank, idx in enumerate(sorted_indices):
        ...
        if (
            is_law_question
            and meta.get("is_law_article", False)
            and meta.get("article_number") in article_numbers
        ):
            # Đảm bảo không vượt quá giới hạn token tối đa
            if (
                total_tokens + estimated_tokens > max_tokens * 1.5
            ):  # Cho phép vượt 50% giới hạn cho điều khoản quan trọng
                token_limit_reached = True
                break
        elif (
            requires_long_answer
            and meta.get("is_important_section", False)
            and rank < 5
        ):
            ...
            if total_tokens + estimated_tokens > max_tokens * 2:
                token_limit_reached = True
                break
        elif requires_overview and (
            "[Granularity: document]" in chunk or "[Granularity: section]" in chunk
        ):
        ...
        original_text = re.sub(
            r"(\s[a-z]\)|\s\([a-z]\))", lambda m: m.group(0), original_text
        )
    ...
    if is_law_question and article_number:
        ...
        for part in context_parts:
            ...
            if (
                f"điều {article_number}" in part_lower
                or f"điều {article_number}." in part_lower
            ):
        ...
        if reordered_context_parts:
            ...
            logger.info(
                f"Đã sắp xếp lại context, đưa {len(reordered_context_parts)} đoạn văn chứa điều {article_number} lên đầu"
            )
...
def rerank_results_for_vietnamese(question, chunks, metadata, top_k=10):
    ...
    keywords = [
        word for word in re.findall(r"\b\w+\b", question.lower()) if len(word) > 1
    ]
    ...
    for i in range(len(chunk_texts)):
        ...
        is_law_question = any(
            re.search(pattern, question.lower()) for pattern in law_patterns
        )
        ...
        if is_law_question and metadata[i].get("is_law_article", False):
            ...
            article_match = re.search(r"điều\s+(\d+)", question.lower())
            if article_match and metadata[i].get(
                "article_number"
...
def split_multiple_questions(question):
    ...
    connector_pattern = r"|".join([rf"({c})" for c in connectors])
    ...
    numbered_questions = re.findall(
        r"(?:\d+[\.\)]\s*|\([a-zA-Z]\)\s*|[a-zA-Z]\)\s*)([^.!?]+\??)", question
    )
    ...
    logger.info(
        f"Đã tách thành {len(filtered_questions)} câu hỏi: {filtered_questions}"
    )
...
def answer_question(
    question, top_k=20, threshold=5.0, model="gemini-2.0-flash", chunking_method=None
):
    """
    ...
    if len(questions) > 1:
        ...
        all_suggested_questions = (
            {}
        )  # Sử dụng dict để lưu gợi ý cho từng câu hỏi không tìm thấy thông tin
        ...
        for i, single_question in enumerate(questions):
            ...
            result = answer_question(
                single_question, top_k, threshold, model, chunking_method
            )
            ...
            if (
                "Không có thông tin liên quan trong tài liệu." in result["answer"]
                and result["suggested_questions"]
            ):
        ...
        if len(question.split()) <= 4:
            ...
            for expanded_question in expanded_questions:
                ...
                q_vec = embedder.encode(
                    expanded_question, convert_to_tensor=False
                ).astype("float32")
                q_vec = np.expand_dims(q_vec, axis=0)

                # Tìm kiếm với FAISS
                distances, indices = faiss_index.search(
                    q_vec, min(top_k, faiss_index.ntotal)
                )
                ...
                if min(distances[0]) < threshold:
                    ...
                    logger.info(
                        f"Tìm thấy kết quả tốt với câu hỏi mở rộng: {expanded_question}"
                    )
    ...
    if len(question.split()) <= 4 and not is_law_question:
        ...
        term_match = re.match(
            r"^([^?]+)(\s+(là gì|gồm|bao gồm|nghĩa là gì|định nghĩa|gồm những gì))?\??$",
            question,
        )
    ...
    for pattern in exact_answer_patterns:
        if re.search(pattern, question.lower()):
            requires_exact_answer = True
            logger.info(
                f"API: Phát hiện câu hỏi yêu cầu trả lời chính xác và đầy đủ: {question}"
            )
    ...
    if is_law_question:
        ...
        top_k = max(
            top_k, 30
        )  # Tăng số lượng chunks cho câu hỏi yêu cầu trả lời chính xác và đầy đủ
    ...
    article_specific_found = (
        False  # Biến cờ để đánh dấu đã tìm thấy điều luật cụ thể chưa
    )
        ...
        if is_law_question and article_number:
            ...
            for idx, meta in enumerate(global_metadata):
                ...
                # Hoặc kiểm tra qua nội dung văn bản
                    ...
                    f"điều {article_number}" in file_text
                    or f"điều {article_number}." in file_text
                ):
                    article_specific_indices.append(idx)
                    article_specific_found = True
            ...
            if article_specific_found:
                logger.info(
                    f"Đã tìm thấy {len(article_specific_indices)} đoạn văn chứa điều luật {article_number}"
                )
    ...
    if unique_indices:
        ...
        if best_distance > threshold * 2:  # Nới lỏng ngưỡng cho hybrid search
            logger.info(
                f"Không tìm thấy chunks phù hợp. Khoảng cách tốt nhất: {best_distance}"
            )
    ...
    for idx, dist in unique_indices.items():
        if idx < len(global_metadata):
            ...
            if is_law_question and global_metadata[idx].get("is_law_article", False):
                ...
                if (
                    article_number
                    and global_metadata[idx].get("article_number") == article_number
                ):
                    # Tăng điểm gấp 3 lần cho điều khoản được đề cập trực tiếp
                    semantic_score *= 3.0
                else:
            ...
            if "[Granularity:" in chunk_text:
                ...
                if requires_overview and "document" in chunk_text:
                    ...
                    semantic_score *= (
                        2.0  # Ưu tiên cao cho section-level chunks khi cần tổng quan
                    )
                elif requires_long_answer and "section" in chunk_text:
                    semantic_score *= (
                        2.0  # Ưu tiên cao cho section-level chunks khi cần trả lời dài
                    )
            ...
            combined_scores[idx] = (
                semantic_score * 0.7
            )  # Trọng số 70% cho tìm kiếm ngữ nghĩa
    ...
    top_indices = [
        idx for idx, _ in sorted_results[: top_k * 2]
    ...
    try:
        ...
        logger.info(
            f"Đã rerank {len(retrieved_chunks)} chunks xuống còn {len(reranked_chunks)} chunks"
        )
    ...
    # Tối ưu hóa context
    context, used_files = build_optimized_context(
        retrieved_chunks, retrieved_metadata, question
    )
...
"""
    ...
    try:
        ...
        vietnamese_patterns = [
            ...
            (
                r"([bcdđghklmnpqrstvxBCDĐGHKLMNPQRSTVX])\s+([aàáạảãăằắặẳẵâầấậẩẫeèéẹẻẽêềếệểễiìíịỉĩoòóọỏõôồốộổỗơờớợởỡuùúụủũưừứựửữyỳýỵỷỹ])",
                r"\1\2",
            ),
    ...
    common_prefixes = [
        ...
        "Dựa vào tài liệu, ",
        "Theo các đoạn văn bản được cung cấp, ",
        "Trả lời: ",
        "Dựa trên các đoạn văn bản, ",
        "Từ thông tin được cung cấp, ",
        "Căn cứ vào tài liệu, ",
        "Theo nội dung tài liệu, ",
        "Dựa vào thông tin trong tài liệu, ",
        "Theo đoạn văn, ",
        "Theo thông tin trong tài liệu, ",
        "Dựa vào các đoạn văn, ",
        "Theo các tài liệu, ",
        "Dựa trên tài liệu, ",
    ...
    if used_files:
        ...
        if "Không có thông tin liên quan trong tài liệu." not in answer:
            ...
            for file_name, chunks in chunks_and_sources.items():
                ...
                content_matched = (
                    False  # Đánh dấu xem có nội dung khớp quan trọng không
                )
                ...
                for chunk in chunks:
                    ...
                    for sentence in answer_sentences:
                        ...
                        if not full_match and len(cleaned_sentence) > 30:
                            ...
                            # Đếm số từ khớp
                            matched_words = sum(
                                1 for word in words if word in chunk_lower
                            )
                    ...
                    if (
                        not content_matched
                    ):  # Chỉ thực hiện nếu chưa tìm thấy khớp hoàn chỉnh
                    ...
                    if not content_matched:
                        ...
                        special_terms += re.findall(
                            r"(\b[A-Za-z]+\b\s+là\s+[^.!?]+)", chunk
                        )  # Mẫu định nghĩa "X là Y"
                ...
                if len(answer_sentences) <= 3 and len(answer_sentences) > 0:
                    main_sentence = answer_sentences[
                        0
                    ]  # Câu đầu tiên thường là câu trả lời chính
                ...
                if (
                    file_score >= 10
                    and content_matched
                    and file_name not in real_sources
                ):
            ...
            sorted_sources = sorted(
                [(s, source_scores.get(s, 0)) for s in real_sources],
                key=lambda x: x[1],
                reverse=True,
            )
            ...
            for file_name, file_score in list(source_scores.items()):
                ...
                if file_name in chunks_and_sources:
                    ...
                    clean_answer_for_compare = re.sub(
                        r"\[Trang \d+\]\s*", "", clean_answer
                    )
                    ...
                    for para in paragraphs:
                        ...
                        if para in clean_all_content:
                            ...
                            source_scores[file_name] += (
                                len(para) / 10
                            )  # Điểm tỷ lệ với độ dài đoạn khớp
        ...
        relevant_pages_dict = identify_most_relevant_pages(
            answer, retrieved_chunks, retrieved_metadata
        )
            ...
            for file_name, pages in relevant_pages_dict.items():
                if file_name in unique_files:
                    ...
                    for meta in retrieved_metadata:
                        if (
                            meta.get("filename") == file_name
                            and str(meta.get("primary_page")) in pages
                        ):
    ...
    logger.info(
        f"Thời gian truy xuất: {retrieval_time:.2f}s, Thời gian trả lời: {answer_time:.2f}s"
    )

    # Theo dõi hiệu suất
    track_performance(
        question, retrieval_time, answer_time, len(retrieved_chunks), chunking_method
    )
    ...
    try:
        ...
        if len(suggested_questions) < 3:
            ...
            suggested_questions.extend(
                default_questions[: (3 - len(suggested_questions))]
            )
...
def view_embeddings():
    ...
    try:
        ...
        for filename in global_all_files:
            doc_chunks = [
                meta for meta in global_metadata if meta.get("source") == filename
            ]
            response["documents"][filename] = {
                "total_chunks": len(doc_chunks),
                "chunking_method": doc_chunks[0].get("chunking_method")
                if doc_chunks
                else "unknown",
...
def api_answer_question():
    ...
    try:
        ...
        if (
            ...
            try:
                result = answer_question(
                    question, reduced_top_k, threshold, model, chunking_method
                )
                return jsonify(
                    {
                        "answer": result["answer"],
        ...
        return jsonify({"error": f"Lỗi khi trả lời câu hỏi: {error_message}"}), 500
...
def load_settings():
    ...
    default_settings = {
        ...
        "paragraph": {"max_chars": 1000, "overlap": 200},
        "semantic": {"min_words": 50, "max_words": 200},
        "token": {"max_tokens": 500, "overlap_tokens": 50},
...
def query():
    ...
    try:
        ...
        if (
            answer
            and "Không có thông tin liên quan trong tài liệu." in answer
            and not suggested_questions
        ):
        ...
        if is_ajax:
            ...
            return render_template_string(
                """<div class="chat-message bot-message"><p class="text-red-500 dark:text-red-400 text-sm sm:text-base">Đã xảy ra lỗi khi xử lý yêu cầu của bạn: {{ error }}</p></div>""",
                error=error_message,
            )
        else:
            return render_template_string(
...
def remove_file():
    ...
    return enhanced_remove_file(
        index_html, global_all_files, load_settings, remove_document
    )
...
def save_settings():
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return (
                jsonify(
                    {
                        "error": "Unauthorized",
                        "message": "Vui lòng đăng nhập để thay đổi cài đặt",
                    }
                ),
                401,
            )
    ...
    try:
        with open(
            os.path.join(upload_folder, "settings.json"), "w", encoding="utf-8"
        ) as f:
...
def identify_most_relevant_pages(answer, retrieved_chunks, retrieved_metadata):
    ...
    if not start_pages:
        return _identify_pages_by_keywords_start(
            unique_sentences, retrieved_chunks, retrieved_metadata
        )
...
def init_user_data():
    ...
    if result["success"]:
        flash(
            f"Khởi tạo dữ liệu thành công! Đã di chuyển {result['migrated_chats']} cuộc trò chuyện và {result['migrated_files']} tệp tin.",
            "success",
        )
    else:
...
def check_supabase_connection():
    ...
    try:
        ...
        results["details"][
            "messages"
        ] = f"Found {len(messages_query.data or [])} records"
        ...
        results["details"]["auth"] = (
            "Logged in as " + user["email"] if user else "Not logged in"
        )
