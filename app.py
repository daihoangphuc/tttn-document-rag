...
#!pip install flask pyngrok google-generativeai transformers sentence-transformers PyPDF2 python-docx nltk faiss-cpu scikit-learn pandas matplotlib tqdm underthesea pyvi rank_bm25 supabase flask-session
...
from flask import Flask, request, render_template_string, redirect, url_for, jsonify, session, flash, render_template
...
from supabase_modules.client import get_supabase_client, test_connection
from supabase_modules import auth
from supabase_modules.storage import StorageManager
from supabase_modules.chat import ChatManager

# Thiết lập logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger('rag_system')

# Khởi tạo dịch vụ Supabase
try:
    # Lấy Supabase client từ module
    supabase_client = get_supabase_client()
    
    # Kiểm tra kết nối
    connection_status, connection_info = test_connection()
    if connection_status:
        logger.info(f"Kết nối Supabase thành công: {connection_info['status']}")
        
        # Khởi tạo các manager
        storage_manager = StorageManager()
        chat_manager = ChatManager()
        logger.info("Đã khởi tạo dịch vụ Supabase thành công")
    else:
        logger.error(f"Kết nối Supabase thất bại: {connection_info.get('error', 'Lỗi không xác định')}")
        supabase_client = None
        storage_manager = None
        chat_manager = None
except Exception as e:
    logger.error(f"Lỗi khi khởi tạo dịch vụ Supabase: {str(e)}")
    supabase_client = None
    storage_manager = None
    chat_manager = None
...
# Cấu hình Flask app cho Session
...
def upload_file():
    ...
    for file in files:
        ...
        try:
            # Lưu file vào thư mục cục bộ hoặc Supabase Storage
            filename = file.filename
            
            if user_id:
                try:
                    # Lưu file vào Supabase Storage
                    file_path = storage_manager.save_user_file(file, user_id)
                    logger.info(f"Đã lưu file {filename} vào Supabase Storage: {file_path}")
                except Exception as storage_error:
                    logger.error(f"Lỗi khi lưu file vào Supabase: {str(storage_error)}")
                    # Nếu lỗi, thử lưu cục bộ vào thư mục người dùng
                    ...
                    file.save(file_path)
                    logger.info(f"Đã lưu file {filename} vào thư mục cục bộ của người dùng: {file_path}")
            else:
                # Lưu file vào thư mục cục bộ chung nếu không có người dùng
                import re
                safe_filename = re.sub(r'[^\w\-\.]', '_', filename)
                file_path = os.path.join(upload_folder, safe_filename)
                file.save(file_path)
                logger.info(f"Đã lưu file {filename} vào thư mục cục bộ chung: {file_path}")
    ...
    # Thông báo cho file trùng lặp
    if duplicate_files:
        if len(duplicate_files) == 1:
            result_message += f"File {duplicate_files[0]} đã tồn tại trong hệ thống. "
        else:
            result_message += f"Các file sau đã tồn tại trong hệ thống: {', '.join(duplicate_files)}. "
    
    # Thông báo cho file xử lý thành công
    ...
    # Thông báo cho file thất bại
    if len(failed_files) > 0:
        result_message += f"Không thể xử lý {len(failed_files)} file: {', '.join(failed_files)}"
    
    # Nếu không có file nào được xử lý
    if len(processed_files) == 0 and len(duplicate_files) == 0 and len(failed_files) == 0:
        result_message = "Không có file nào được xử lý."
...
def remove_file():
    ...
    try:
        # Xóa file trong Supabase nếu người dùng đã đăng nhập
        current_user = auth.get_current_user()
        if current_user:
            try:
                storage_success = storage_manager.delete_user_file(filename)
                if storage_success:
                    logger.info(f"Đã xóa file {filename} khỏi Supabase Storage")
                else:
                    logger.warning(f"Không tìm thấy file {filename} trong Supabase Storage")
            except Exception as storage_error:
                logger.error(f"Lỗi khi xóa file từ Supabase: {str(storage_error)}")
        
        # Xóa file khỏi hệ thống RAG
...
def initialize_system():
    ...
    try:
        ...
        if supabase_connected:
            logger.info("Kết nối đến Supabase thành công")
        else:
            logger.warning("Không thể kết nối đến Supabase, sẽ sử dụng lưu trữ cục bộ")
    except Exception as e:
        logger.error(f"Lỗi khi kiểm tra kết nối Supabase: {str(e)}")
        logger.warning("Sẽ sử dụng lưu trữ cục bộ do không thể kết nối Supabase")
    
    # Các file cần thiết
    rag_state_path = os.path.join(upload_folder, 'rag_state.json')
    vectors_path = os.path.join(upload_folder, 'vectors.pkl')
    faiss_index_path = os.path.join(upload_folder, 'faiss_index.bin')
    tfidf_vectorizer_path = os.path.join(upload_folder, 'tfidf_vectorizer.pkl')
    tfidf_matrix_path = os.path.join(upload_folder, 'tfidf_matrix.pkl')
    
    # Kiểm tra xem các file cần thiết đã tồn tại chưa
    if (os.path.exists(rag_state_path) and 
        os.path.exists(vectors_path) and 
        os.path.exists(faiss_index_path) and 
        os.path.exists(tfidf_vectorizer_path) and 
        os.path.exists(tfidf_matrix_path)):
        
        logger.info("Phát hiện dữ liệu đã tồn tại. Đang tải lại...")
        # Tải lại dữ liệu
        load_state()
        logger.info(f"Đã tải lại dữ liệu thành công: {len(global_metadata)} chunks, {len(global_all_files)} files")
    else:
        logger.info("Không phát hiện dữ liệu đã lưu. Khởi động hệ thống mới.")
    
    # Đảm bảo bucket tồn tại nếu kết nối Supabase thành công
    try:
        if storage_manager:
            storage_manager.ensure_bucket_exists()
    except Exception as e:
        logger.error(f"Lỗi khi tạo bucket Supabase: {str(e)}")
    
    # Kiểm tra nếu người dùng đã đăng nhập, tải dữ liệu của họ
    try:
        from flask import has_request_context
        if has_request_context() and auth.is_authenticated():
            try:
                initialize_user_system()
            except Exception as e:
                logger.error(f"Lỗi khi khởi tạo dữ liệu người dùng: {str(e)}")
    except Exception as e:
        logger.warning(f"Không thể kiểm tra trạng thái xác thực: {str(e)}")
...
# --- Các route xử lý xác thực người dùng ---

@app.route('/login', methods=['GET', 'POST'])
def login():
    """
    Tuyến đường xử lý đăng nhập người dùng
    """
    # Lấy URL chuyển hướng từ tham số truy vấn
    ...
    if request.method == 'POST':
        ...
        if result['success']:
            ...
            # Khởi tạo hệ thống cho người dùng
            initialize_user_system()
            ...
            flash(f'Đăng nhập thất bại: {result.get("error", "Vui lòng kiểm tra lại email và mật khẩu")}', 'error')
...
def register():
    """
    Tuyến đường xử lý đăng ký người dùng mới
    """
    # Nếu người dùng đã đăng nhập, chuyển hướng đến trang chính
    if 'user_id' in session:
    ...
    if request.method == 'POST':
        ...
        if not username:
            flash('Vui lòng nhập tên người dùng', 'error')
            return render_template('register.html')
        ...
        try:
            ...
            logger.info(f"Đang đăng ký người dùng mới với email: {email} và username: {username}")
            ...
            if result['success']:
                ...
                flash(f'Đăng ký thất bại: {result.get("error", "Vui lòng thử lại")}', 'error')
                logger.error(f"Lỗi đăng ký: {result.get('error', 'Không có thông tin lỗi')}")
        except Exception as e:
            logger.error(f"Lỗi không xác định khi đăng ký: {str(e)}")
...
def logout():
    """
    Tuyến đường xử lý đăng xuất người dùng
    """
    auth.sign_out()
...
def initialize_user_system():
    """
    Khởi tạo hệ thống cho người dùng khi đăng nhập
    
    Hàm này sẽ tải lại các dữ liệu của người dùng, bao gồm:
    - Lịch sử chat
    - Tài liệu đã tải lên
    """
    try:
        ...
        user_email = session.get('user_email', user_id)
        logger.info(f"Khởi tạo hệ thống cho người dùng: {user_email}")
        
        # Tạo thư mục riêng cho người dùng nếu chưa có
        if storage_manager:
            try:
                user_folder = storage_manager.get_user_folder(user_id)
                
                # Tải lại các tài liệu đã tải lên
                user_files = storage_manager.list_files(user_folder)
                logger.info(f"Đã tìm thấy {len(user_files)} file của người dùng {user_email}")
            except Exception as storage_error:
                logger.error(f"Lỗi khi truy cập storage: {str(storage_error)}")
        
        # Tải lại lịch sử chat
        if chat_manager:
            try:
                user_chats = chat_manager.get_user_conversations(user_id)
                
                # Đặt chat hiện tại là chat mới nhất (nếu có)
                if user_chats:
                    session['current_chat_id'] = user_chats[0]['id']
                    logger.info(f"Đã tải {len(user_chats)} cuộc hội thoại của người dùng {user_email}")
            except Exception as chat_error:
                logger.error(f"Lỗi khi truy cập chat: {str(chat_error)}")
        
        logger.info(f"Đã khởi tạo hệ thống cho người dùng {user_email}")
        return True
    except Exception as e:
        logger.error(f"Lỗi khi khởi tạo hệ thống cho người dùng: {str(e)}")
        return False
...
if __name__ == "__main__":
    ...
    logger.info("Khởi động server Flask")
    
    try:
        # Thử sử dụng ngrok nếu có
        ngrok_token = os.getenv("NGROK_AUTH_TOKEN")
        if ngrok_token:
            try:
                import pyngrok.ngrok as ngrok
                ngrok.set_auth_token(ngrok_token)  # Lấy token từ biến môi trường
                public_url = ngrok.connect(int(os.getenv("PORT", 5000)))
                print("Public URL:", public_url)
            except Exception as e:
                logger.error(f"Lỗi khi khởi tạo ngrok: {str(e)}")
                print("Không thể khởi tạo ngrok, chỉ chạy local")
    except ImportError:
        logger.warning("Không tìm thấy module pyngrok, chỉ chạy local")
